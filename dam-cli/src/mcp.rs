...
impl DamMcpServer {
    ...
    fn resolve_token(
        ...
        let parsed: dam_core::Token = match inner.parse() {
            Ok(t) => t,
            Err(e) => return Ok(CallToolResult::error(vec![Content::text(format!("Invalid token: {e}"))])),
        };
        ...
        match self.vault.retrieve(&parsed) {
            ...
            Err(e) => Ok(CallToolResult::error(vec![Content::text(format!("Resolution failed: {e}"))])),
        }
    }
    ...
    fn list_tokens(
        ...
        let filter = data_type.as_deref().and_then(dam_core::SensitiveDataType::from_tag);
        ...
        let result: Vec<_> = entries.iter().map(|e| json!({
            "token": e.ref_id,
    ...
    fn grant_consent(
        ...
        match self.consent.grant(&data_type, token_key, &destination, ConsentAction::Pass, ttl_secs) {
            Ok(rule) => {
    ...
    fn deny_consent(
        ...
        match self.consent.grant(&data_type, token_key, &destination, ConsentAction::Redact, None) {
            Ok(rule) => Ok(CallToolResult::success(vec![Content::text(json!({
                "rule_id": rule.id,
                "data_type": rule.data_type,
                "destination": rule.destination,
                "action": "redact",
    ...
    fn revoke_consent(
        ...
        match self.consent.revoke(&rule_id) {
            Ok(true) => Ok(CallToolResult::success(vec![Content::text(json!({
                "revoked": true,
                "rule_id": rule_id,
            }).to_string())])),
            Ok(false) => Ok(CallToolResult::error(vec![Content::text(format!("Rule not found: {rule_id}"))])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(format!("Error: {e}"))])),
        }
    }
    ...
    fn list_consent(&self) -> Result<CallToolResult, McpError> {
        match self.consent.list() {
            Ok(rules) => {
                let result: Vec<_> = rules.iter().map(|r| json!({
                    "rule_id": r.id,
    ...
    fn get_stats(&self) -> Result<CallToolResult, McpError> {
        match self.log.stats() {
            Ok(stats) => {
                let result: Vec<_> = stats.iter().map(|s| json!({
