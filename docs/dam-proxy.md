...
`dam-proxy` is the first hot-path proxy module. It is an application-layer OpenAI-compatible endpoint / reverse proxy for selected LLM traffic. It does not do TLS interception, local CA installation, WebSockets, VPN/TUN routing, or arbitrary web traffic rewriting.
...
- `config_required`: returned when a target requires an API key env var, the env var is missing, and the incoming request has no `Authorization` header.
