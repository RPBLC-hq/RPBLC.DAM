...
The provider resolves `sourceAppAuditToken` to PID and process path, combines that with `sourceAppSigningIdentifier`, and bypasses DAM-owned traffic by stable signing identifier, full path, or legacy binary basename. Packaged builds sign nested DAM executables with explicit identifiers such as `com.rpblc.dam.proxy`, `com.rpblc.dam.web`, `com.rpblc.dam.daemon`, `com.rpblc.dam.cli`, `com.rpblc.dam.mcp`, and `com.rpblc.dam.tray`; the provider also keeps legacy basename exclusions like `dam-proxy` and `dam-web` for older or development builds. For configured flows handed to DAM, the synthesized CONNECT preface includes sanitized `X-DAM-Source-*` metadata headers for internal diagnostics and future profile decisions.
