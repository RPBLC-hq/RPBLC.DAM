...
- Transparent interception is HTTP/1.1 CONNECT/TLS for active traffic profile hosts, configured OpenAI-compatible and Anthropic targets, no HTTP/2, and no chunked request bodies. Intercepted JSON and `text/event-stream` responses are transformed for inbound reference resolution when global restoration is enabled and the matched traffic app allows it, including provider-aware text-delta event streams for OpenAI-compatible and Anthropic targets.
- Codex ChatGPT-login support keeps inbound restoration disabled for `chatgpt.com` and `ab.chatgpt.com` HTTP/WebSocket backend traffic. Its WebSocket support protects unfragmented client and server text frames, including raw provider-returned values and related domains learned from the outbound request context. Fragmented, binary, or compressed frame protection remains unsupported and logs warning events when those frames pass through without mutation.
